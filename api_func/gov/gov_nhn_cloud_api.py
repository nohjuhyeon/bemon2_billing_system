...
def check_token(access_key, token):
    ...
    # API URL 및 쿼리 파라미터
    uri = 'https://core.api.nhncloudservice.com/v1/authentications/user-access-keys/{}/tokens'.format(access_key)
    query_params = {
        'status':'ACTIVE'
    }
    ...
    # 요청 헤더
    headers = {
        "x-nhn-authorization": f"Bearer {token}"  # 토큰을 적절한 형식으로 전달
    }
    ...
    # 디버깅용 URL 출력
    response_url = requests.Request('GET', uri, headers=headers, params=query_params).prepare().url
